...
fn keywords_and_identifiers() {
    ...
    assert!(toks.iter().any(|(t, _)| matches!(t, Token::Ident(s) if s == "letx")));
    assert!(toks.iter().any(|(t, _)| matches!(t, Token::Ident(s) if s == "iffy")));
}
...
fn numbers_and_floats() {
    ...
    let toks = hyperlight::tokenize(src).expect("tokenize");
    let nums: Vec<_> = toks.into_iter().filter_map(|(t, _)| match t { Token::Number(n) => Some(n), _ => None }).collect();
    assert!(matches!(nums[0], hyperlight::token::NumberLit::Int(0)));
    assert!(matches!(nums[1], hyperlight::token::NumberLit::Int(123)));
    assert!(matches!(nums[2], hyperlight::token::NumberLit::Float(f) if (f - 3.1415).abs() < 1e-12));
}
