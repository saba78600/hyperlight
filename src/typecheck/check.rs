use super::env::{TypeEnv, TypeError};
    ...
    for s in stmts {
        match s {
            ...
            Stmt::Assign { name, value } => {
                ...
                let var_ty = env
                    .get(name)
                    .ok_or(TypeError::UnknownIdentifier(name.clone()))?
                    .clone();
        ...
        Expr::Call { callee, args } => {
            ...
            if callee == "print" {
                if args.len() != 1 {
                    return Err(TypeError::Mismatch {
                        expected: Type::Void,
                        found: Type::Custom("argcount".into()),
                    });
                }
